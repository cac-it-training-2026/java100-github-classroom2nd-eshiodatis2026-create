...
public class WarehouseManager {
	...
	public static void main(String[] args) throws IOException {
		...
		int[] jankenArray1 = new int[5];//Yさん
		int[] jankenArray2 = new int[5];//Sさん

		//Yさんのコンテナにじゃんけんの手を入れる処理を記述する。
		int num = 0;//ユーザが入力した手、Sさんのランダムな手を一時的に保存するための作業用変数
		boolean loopFlag = false;//ループを続けるかどうかの目印。do-while文と使う。
		//falseはループを終了、trueはもう一度
		...
		for (int i = 0; i < jankenArray1.length; i++) {
			...
			} while (loopFlag);//do-while文の条件判定部分
			jankenArray1[i] = num;//i番目のコンテナに確定した手numを入れる（グーチョキパーで2回使われていない手）
			if (num == 1) {//Sさんが出した手に応じてそれぞれのカウンタを＋1する
				stoneCount1++;
			} else if (num == 2) {
				scissorsCount1++;
			} else {
		...
		for (int count = 0; count < 5; count++) {
			...
			do {
				...
				String kontena = br.readLine();
				kontena1 = Integer.parseInt(kontena) - 1;
				if (kontena1 < 0 || kontena1 > 4) {
					System.out.println("Yさん");
					System.out.println("え～。そんなコンテナないよ。");
					loopFlag = true;

				} else if (jankenArray1[kontena1] == 0) {
					System.out.println("Yさん");
			...
			do {
				loopFlag = false;
				kontena2 = (int) (Math.random() * 10) % 5;
				if (jankenArray2[kontena2] == 0) {
					loopFlag = true;
				}
			} while (loopFlag);

			System.out.println("\nYさん：");
			System.out.println("よし、" + (kontena1 + 1) + "番コンテナだ！\n");

			System.out.println("Sさん：");
			System.out.println("こっちは" + (kontena2 + 1) + "番コンテナだべ！\n");

			System.out.println("審判：");
			System.out.println("ではオープンしてみましょう。\n");

			System.out.println("審判：");
			System.out.print("Yさん：" + handArray[jankenArray1[kontena1] - 1]);
			System.out.print(" VS ");
			System.out.println("Sさん：" + handArray[jankenArray2[kontena2] - 1]);
			System.out.print("で、");

			//1回ごとの勝敗判定およびメッセージの出力処理を記述する。
			if (jankenArray1[kontena1] == jankenArray2[kontena2]) {
				...
				if (jankenArray1[kontena1] == 1) {
					if (jankenArray2[kontena2] == 2) {
						...
						winPoint++;
					} else if (jankenArray2[kontena2] == 3) {
				...
				} else if (jankenArray1[kontena1] == 2) {
					if (jankenArray2[kontena2] == 1) {
						System.out.println("Sさんの勝ちです。\n");
						System.out.println("Yさん：");
						System.out.println("次は勝つぞ！\n");
						System.out.println("Sさん：");
					...
					if (jankenArray2[kontena2] == 1) {
						...
						System.out.println("あー、もうやんだぐなっできた...\n");
						winPoint++;
					} else if (jankenArray2[kontena2] == 2) {
						System.out.println("Sさんの勝ちです。\n");
						System.out.println("Yさん：");
						System.out.println("次は勝つぞ！\n");
						System.out.println("Sさん：");
			...
			jankenArray1[kontena1] = 0;
			jankenArray2[kontena2] = 0;
		}
