...
class Spaceship {
	...
	public boolean checkFuel(double fuelNum) {
		...
		double fuel = Math.floor(fuelNum);//Math.floor→小数点以下を切り捨ててそれ以下で一番大きい整数にするメソッド
		if (fuel >= 12) {//燃料が１２以上ならtrue
			isCheck = true;
		}
		return isCheck;
	}
...
public class Astronaut {
	...
	public static void main(String[] args) throws IOException {
		...
		double fuelNum = Double.parseDouble(fuelStr);//文字列→doubleに変換
